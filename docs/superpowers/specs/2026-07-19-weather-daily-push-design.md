...
- **做**：当日天气（天气状况 + 最高/最低温）、DeepSeek 穿衣建议、定点推送、多城市合并。
...
沿用项目依赖注入风格：`NewWeather(httpClient *http.Client, notifier notify.Notifier, advisor Advisor) *Weather`，
由 `main` 组装并注入现有的 `httpClient`、`tgClient`、`aiClient`。
...
所有动态片段（城市名、状况、温度、建议文本）用 `tools.EscapeMarkdownV2` 转义；
...
拼装逻辑放纯函数 `buildMessage(items []cityWeather, advice string) string`，便于单测。
...
- `buildMessage(items, advice)`：断言含各城市、转义正确、有/无建议两种情况。
...
- 改 `cmd/news2tg/main.go`（组装并注册 weather monitor）
- 改 `config.toml` / `myconfig.toml`（模板与本地配置示例）
