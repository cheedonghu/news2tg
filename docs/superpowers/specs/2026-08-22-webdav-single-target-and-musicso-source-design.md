...
**未能验证**：非中国 IP + 浏览器 UA 是否放行。手头唯一的境外探针无法设置浏览器 UA，
它返回的 403 只反映 UA、不反映地域，不能作为地域封锁的证据。

**主要风险不是地域，是 Cloudflare 的 bot 评分**：它对机房/VPS 的 ASN 明显比对住宅
IP 更严。也就是说"部署到任意一台 VPS"（无论国内国外）撞上质询的概率都高于开发机，
这与国别关系不大。`[network] proxy` 是这种情况下的出口；代码侧则必须保证质询发生时
错误信息说得清楚（见下）。
